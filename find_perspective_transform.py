...
straight_images = ["pers_transform\straight_lines1.jpg", "pers_transform\straight_lines2.jpg"]
...
for img_path in straight_images:
    ...
    for line in lines:
        for x1, y1, x2, y2 in line:
            ...
            image = cv2.line(img, (x1,y1), (x2, y2),(255, 0, 0), thickness=2)
            '''
...
red = [0,0,255]
image = cv2.imread("pers_transform\straight_lines1.jpg")

image = cv2.circle(image, (639,419), radius = 4, color = (0,0,255), thickness = -1)
cv2.imshow("Image",image)
cv2.waitKey(0)
...
cv2.polylines(image, [src_points.astype(np.int32)],True, (0,0,255), thickness=2)
cv2.imshow("Image",image)
cv2.waitKey(0)
